...
use Lwt\Services\TextReadingService;
...
function echo_term(
    ...
    $service = new TextReadingService();
    $service->echoTerm($actcode, $showAll, $spanid, $hidetag, $currcharcount, $record, $exprs);
}
...
{
    $service = new TextReadingService();
    return $service->parseSentence($sid, $old_sid);
}
...
function item_parser(
    ...
    $service = new TextReadingService();
    $service->parseItem($record, $showAll, $currcharcount, $hide, $exprs);
}
...
{
    $service = new TextReadingService();
    $service->mainWordLoop($textid, $showAll);
}
